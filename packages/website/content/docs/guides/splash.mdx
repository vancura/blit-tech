---
# Generated from blit386/docs/guide-splash.md by scripts/sync-docs-from-engine.mjs.
...
the three `configure()` fields are in the [hardware settings table](/docs/api/core#hardware-settings).
...
already blackened, then brings it up with [`BT.paletteFadeExposure`](/docs/api/palette#exposure-fade). The splash fading
...
[API: Core](/docs/api/core#reduced-motion)).
